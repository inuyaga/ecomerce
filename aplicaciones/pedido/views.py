...
class CarritoLista(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['ped_lim_ofi'] = DetallePedido.objects.filter(dtl_creado_por=self.request.user, dtl_status=False, dtl_tipo_pedido=8)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if tipo_pedido == 'ped_lim_ofi':
            ...
            if pedido_count == 0:
                pedido.save()
                DetallePedido.objects.filter(dtl_creado_por=self.request.user, dtl_status=False, dtl_tipo_pedido=8).update(dtl_status=True, dtl_id_pedido=pedido)
...
class EntregaUpdate(LoginRequiredMixin, ListView):
    login_url = '/login/'
    redirect_field_name = 'redirect_to'
    model = Sucursal
    template_name = 'pedido/entregadoView.html'
    ...
    @method_decorator(permission_required('pedido.change_pedido', reverse_lazy('requiere_permisos')))
    def dispatch(self, *args, **kwargs):
        return super(EntregaUpdate, self).dispatch(*args, **kwargs)

    def get_queryset(self):
        queryset = super(EntregaUpdate, self).get_queryset()
        busqueda=self.request.GET.get('buscar')
        if busqueda != None:
            queryset=queryset.filter(Q(suc_nombre__icontains=busqueda) | Q(suc_zona__zona_nombre__icontains=busqueda))
        return queryset

class EntregaEntregar(View):
    def get(self, request, *args, **kwargs):
        import datetime
        ahora = datetime.datetime.now()
        id_suc = self.kwargs.get('pk')
        Pedido.objects.filter(ped_id_Suc=id_suc, ped_estatusPedido=6, ped_entregado=False).update(ped_fechaEntrega=ahora, ped_entregado=True)
        rev = reverse_lazy('pedido:pedido_entrega')
        return redirect(rev)

class EntregaEntregados(LoginRequiredMixin,ListView):
    login_url = '/login/'
    redirect_field_name = 'redirect_to'
    model = Pedido
    template_name = 'pedido/pedido_entregado.html' 

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['usuario'] = self.request.user
        if self.request.GET:
            context['form_filtro'] = FiltroFormEntrega(self.request.GET)
        else:
            context['form_filtro'] = FiltroFormEntrega()
        return context

    def get_queryset(self): 

        queryset = super(EntregaEntregados, self).get_queryset()
        status = 6
        tipo_pedido = self.request.GET.getlist('tipo_pedido')
        inicio = self.request.GET.get('inicio')
        fin = self.request.GET.get('fin')
        ...
        if self.request.user.is_superuser or self.request.user.tipo_user == 2:
            if len(self.request.GET) == 0:
                queryset = queryset.filter(ped_estatusPedido=6, ped_entregado=True)

            if status != '0':
                queryset = queryset.filter(ped_estatusPedido=status, ped_entregado=True)
            
            if inicio !=None and fin != None:
                queryset=queryset.filter(ped_fechaCreacion__range=(inicio, fin), ped_entregado=True)
            
            if tipo_pedido:                
                queryset = queryset.filter(dtl_tipo_pedido__in=tipo_pedido, ped_entregado=True)
