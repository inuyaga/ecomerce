...
TIPO_PEDIDO = (
(1, "Papeleria"), (2, "Limpieza"), (3, "Limpieza Consultorio"), (4, "Toner"), (5, "Papeleria consultorio"),
(6, 'Toner consultorio'), (7, 'Globos'), (8, 'Limpieza oficina'))
...
class Producto(models.Model):
    ...
    prod_v_limpieza_oficina = models.BooleanField('Visible Limpieza oficina', default=False)
...
class Pedido(models.Model):
    ...
    ped_id_UsuarioCreo = models.ForeignKey(User, verbose_name="Creador de pedido", related_name='creador',
                                           on_delete=models.SET_NULL, blank=True, null=True)
    ped_id_UsuarioAutorizo = models.ForeignKey(User, verbose_name="Autorizador", related_name='autorizador',
                                               on_delete=models.SET_NULL, blank=True, null=True)
    ped_id_UsuarioCancelo = models.ForeignKey(User, verbose_name="Cancelador", related_name='cancelo',
                                              on_delete=models.SET_NULL, blank=True, null=True)
    ped_id_usr_descargo_excel = models.ForeignKey(User, verbose_name="Descargo", related_name='descargo',
                                                  on_delete=models.SET_NULL, blank=True, null=True)
    ...
    ped_entregado = models.BooleanField('Entregado', default=False)
    ped_fechaEntrega = models.DateTimeField("Fecha de entrega", blank=True, null=True)
    ...
    def total_venta(self):
        query = DetallePedido.objects.filter(dtl_id_pedido=self.ped_id_ped).aggregate(
            total_venta=Sum(F('dtl_cantidad') * F('dtl_precio'), output_field=FloatField()))
        return 0 if query['total_venta'] == None else round(query['total_venta'], 3)


class DetallePedido(models.Model):
    dtl_id_detalle = models.AutoField(primary_key=True)
    dtl_id_pedido = models.ForeignKey(Pedido, verbose_name="Folio Pedido", blank=True, null=True,
...
class ConfigRestriccion(models.Model):
    conf_r_fecha_inicio = models.DateField('Fecha inicio')
    conf_r_fecha_fin = models.DateField('Fecha final')
    conf_r_tipo_pedido = models.IntegerField(choices=TIPO_PEDIDO, default=0)

    # def __str__(self):
    #     return str(self.get_conf_r_tipo_pedido_display)
